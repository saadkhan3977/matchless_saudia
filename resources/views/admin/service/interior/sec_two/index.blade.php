...
<style type="text/css">
      ...
      <div class="padding">
        <div class="box" style="height: 250px;">
          <div class="box-header">
            <h3>Manage Interior Section 2 Heading</h3><hr>
            @if(count($data) < '2')
                  ...
                  <div class="modal-body">
                    <div class="tab-content">
                      <div class="tab-pane p-v-sm  @if(!$en && $ar) active @endif " id="tab_1">
                        <form role="form" method="post" action="{{url('/admin/interior/sectwo_heading')}}" enctype="multipart/form-data">
                          @csrf
        ...
        <div class="table-responsive">
          ...
          <tr>
            ...
            <td style="display: flex;">
                  ...
                  <div class="modal-body">
                    <div class="tab-content">
                      <div class="tab-pane p-v-sm  @if($row->ar) active @endif " id="tabb_{{$row->id}}">
                        <form role="form" method="post" action="{{url('/admin/interior/sectwo_heading/'.$row->id)}}" enctype="multipart/form-data">
                          @csrf
                          @method('put')
                          <div class="form-group">
                            <label for="exampleInputEmail1">Title</label>
              ...
              <form method="post" action="{{url('/admin/interior/sectwo_heading/'.$row->id)}}">
                @csrf
                @method('delete')
  ...
  <div class="box">
          <div class="box-header">
            <h3>Manage Home Section 2</h3><hr>
            @if(count($sectwo) < '6')
              <a href="{{route('interior_sec_two.create')}}" class="btn btn-icon generalsetting_admin">
        ...
        <div class="table-responsive" style="height: 100%">
          ...
          <tr>
            <td>{{$row->title}}</td>
            <td>{{$row->description}}</td>
            <td><img src="/uploads/interior/{{$row->image}}" alt="{{$row->title}}" width="100"></td>
            <td>{{$row->lang}}</td>
            <td style="display: flex;">
              <a style="margin-right: 10px;" class="btn btn-default generalsetting_admin" href="{{route('interior_sec_two.show',$row->id)}}"><i class="fa fa-edit "></i></a>
              <form method="post" action="{{route('interior_sec_two.destroy',$row->id)}}">
                @csrf
                @method('delete')
              <button type="submit" onclick="return confirm('Are You Sure Want To Delete This..??')" class="btn btn-default generalsetting_admin"><i class="fa fa-trash "></i></button>
              </form>
