    ...
    <div ui-view class="app-body" id="view">
      ...
      <div class="padding">
          ...
          <div class="box-header">
            <h3>Manage Blog</h3>
            <hr>
              <a href="{{route('blog.create')}}" class="btn btn-icon generalsetting_admin"><i class="fa fa-plus"></i></a>
          </div>
      ...
      <table class="table table-striped b-t">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{$row->title}}</td>
            <td>{{$row->company}}</td>
            <td><img width="150" height="100" src="/uploads/blog/{{$row->image}}" alt="{{$row->title}}"></td>
            <td>{{ \Illuminate\Support\Str::limit($row->description, 100, '...') }}</td>
            <td>{!! \Illuminate\Support\Str::limit($row->content, 300, '...') !!}</td>
            <td style="display: flex;">
              <a style="margin-right: 10px;" class="btn btn-default generalsetting_admin" href="{{route('blog.show',$row->id)}}"><i class="fa fa-edit "></i></a>
